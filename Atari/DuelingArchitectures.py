...
for episode in tqdm(range(num_episodes)):
    ...
    while not done:
        ...
        if episode > replay_start_size:
            ...
            q_pred_v, q_pred_a = agent.online_network.forward(T.tensor(states).to(agent.online_network.device, dtype=T.float))
            
            q_pred = q_pred_v.reshape(agent.mini_batch_size) + (q_pred_a[inds, actions] - q_pred_a.mean(1))

            # Dueling Double DQN
            q_eval_v, q_eval_a = agent.online_network.forward(
            ...
            q_next_v, q_next_a = agent.target_network.forward(
                T.tensor(states_).to(
                    agent.target_network.device, dtype=T.float)
                )

            q_next = q_next_v.reshape(agent.mini_batch_size) + (q_next_a[inds, max_actions] - q_next_a.mean(1))
