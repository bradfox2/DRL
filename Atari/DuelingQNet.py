...
class DualDeepQCNN(nn.Module):
    def __init__(self, input_dims, n_actions, lr=0.0001, name='NN', chkpt_dir='./nn.nn'):
        ...
        self.fcAdv = nn.Linear(512, n_actions)
    ...
    def forward(self, data):
        ...
        value_stream = self.fcValue(fc1)
        advantage_stream = self.fcAdv(fc1)

        return value_stream, advantage_stream
        
        avg_state_action_advantage = advantage_stream.mean(1)

        advantage_stream_normalized = advantage_stream - avg_state_action_advantage

        return value_stream + advantage_stream_normalized
